...
public class SpawnSwoopers extends EnemyWave {

	boolean spawned = false;

	public SpawnSwoopers(EndlessLevel level) {
		super(level);
	}
	...
	public void update(float deltaTime) {
		if (!spawned) {
			spawned = true;
			int numToSpawn = Assets.rand.nextInt(3) + 1 + (int) level.sector
					/ 2;
			for (int i = 0; i < numToSpawn; i++) {
				Enemy e = new Swooper(Assets.pilot.level);
	...
	public String getDescription() {
		return "The next sector features opponents whose shots will home in on your ship.";
	}
